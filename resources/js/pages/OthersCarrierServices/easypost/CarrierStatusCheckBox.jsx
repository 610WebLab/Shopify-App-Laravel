...
const CarrierStatusCheckBox = ({ defaultValue, methodId }) => {
    const { show } = useToast();
    ...
    const handleZoneMethodChange = useCallback((newChecked) => { 
        ...
        fetch("/update-carrier-status/"+methodId+"?shop=" + Config.shop+"&status="+status).then(res => res.json()).then((result) => {
...
export default CarrierStatusCheckBox
