...
const FulfilledOrder = () => {
  ...
  const getOrderData = (page, queryValue) => {
    setLoader(false);
    fetch("/orders?shop=" + Config.shop + "&page=" + page + "&search=" + queryValue + "&type=Fulfilled")
      .then((res) => res.json())
      .then(
        (result) => {
          setLoader(true);
  ...
  const rowMarkup = orders.map(
    (
      { id, customer_email, label_url, customer_id, customer_name, date, delivery_method, delivery_status, fullfilement, item_count, labels, order_id, order_name, order_no, payment_status, tags, total, user_id, created_at }, index,) => (
      <IndexTable.Row
        id={id}
        key={id}
      ...
      >
        ...
        <IndexTable.Cell>
          <Button
            ...
            disabled={!label_url}
          >
            {/* <Icon source={FulfillmentFulfilledMajor} tone="base" /> Fullfill */}
            Print Label
          </Button>
        </IndexTable.Cell>
